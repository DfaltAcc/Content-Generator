...
// Also handle the /api/auth/status call made by the useEffect on mount
beforeAll(() => {
  global.fetch = jest.fn().mockImplementation((url: string) => {
    if (url === '/api/auth/status') {
      return Promise.resolve({
        ok: true,
        json: async () => ({ linkedin: { connected: false }, twitter: { connected: false } }),
      });
    }
...
/** Click a tab button by its visible label, scoped to the given container */
function clickTab(label: string, container?: HTMLElement) {
  const scope = container ? within(container) : screen;
  fireEvent.click(scope.getByRole('tab', { name: label }));
}
    ...
    fc.assert(
      fc.property(
        ...
        (contentType, topic, tone) => {
          ...
          act(() => clickTab('Image Prompt', container));

          // Switch back to Text tab
          act(() => clickTab('Text', container));
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (contentType, topic, tone) => {
          ...
          // Verify fetch was called for /api/generate
          const generateCalls = fetchMock.mock.calls.filter(([url]: [string]) => url === '/api/generate');
          expect(generateCalls).toHaveLength(1);

          // Extract the request body
          const [, callOptions] = generateCalls[0] as [string, RequestInit];
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (depiction) => {
          ...
          const generateCallsImg = fetchMock.mock.calls.filter(([url]: [string]) => url === '/api/generate');
          expect(generateCallsImg).toHaveLength(1);

          const [, callOptions] = generateCallsImg[0] as [string, RequestInit];
          const body = JSON.parse(callOptions.body as string) as GenerateRequest;
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (recipient, purpose, keyPoints) => {
          ...
          const generateCallsEmail = fetchMock.mock.calls.filter(([url]: [string]) => url === '/api/generate');
          expect(generateCallsEmail).toHaveLength(1);

          const [, callOptions] = generateCallsEmail[0] as [string, RequestInit];
          const body = JSON.parse(callOptions.body as string) as GenerateRequest;
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (tab, value) => {
          ...
          const generateCallsTab = fetchMock.mock.calls.filter(([url]: [string]) => url === '/api/generate');
          expect(generateCallsTab).toHaveLength(1);

          const [, callOptions] = generateCallsTab[0] as [string, RequestInit];
          const body = JSON.parse(callOptions.body as string) as GenerateRequest;
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ contentType, topic, tone }) => {
          ...
          const generateCallsP6Text = fetchMock.mock.calls.filter(([url]: [string]) => url === '/api/generate');
          expect(generateCallsP6Text).toHaveLength(1);
          const [, firstCallOptions] = generateCallsP6Text[0] as [string, RequestInit];
          ...
          const generateCallsP6TextAfter = fetchMock.mock.calls.filter(([url]: [string]) => url === '/api/generate');
          expect(generateCallsP6TextAfter).toHaveLength(2);
          const [, secondCallOptions] = generateCallsP6TextAfter[1] as [string, RequestInit];
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ depiction }) => {
          ...
          const generateCallsP6Img = fetchMock.mock.calls.filter(([url]: [string]) => url === '/api/generate');
          expect(generateCallsP6Img).toHaveLength(1);
          const [, firstCallOptions] = generateCallsP6Img[0] as [string, RequestInit];
          const firstBody = JSON.parse(firstCallOptions.body as string) as GenerateRequest;
          ...
          const generateCallsP6ImgAfter = fetchMock.mock.calls.filter(([url]: [string]) => url === '/api/generate');
          expect(generateCallsP6ImgAfter).toHaveLength(2);
          const [, secondCallOptions] = generateCallsP6ImgAfter[1] as [string, RequestInit];
          const secondBody = JSON.parse(secondCallOptions.body as string) as GenerateRequest;
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ recipient, purpose, keyPoints }) => {
          ...
          const generateCallsP6Email = fetchMock.mock.calls.filter(([url]: [string]) => url === '/api/generate');
          expect(generateCallsP6Email).toHaveLength(1);
          const [, firstCallOptions] = generateCallsP6Email[0] as [string, RequestInit];
          const firstBody = JSON.parse(firstCallOptions.body as string) as GenerateRequest;
          ...
          const generateCallsP6EmailAfter = fetchMock.mock.calls.filter(([url]: [string]) => url === '/api/generate');
          expect(generateCallsP6EmailAfter).toHaveLength(2);
          const [, secondCallOptions] = generateCallsP6EmailAfter[1] as [string, RequestInit];
          const secondBody = JSON.parse(secondCallOptions.body as string) as GenerateRequest;
